...
def complex_to_real(data):
    ...
    y = fftshift(y, dim=(-2,-1))  ## (1,1,h,w)
    y = torch.fft.irfftn(y,s=(h,w),dim=(-2,-1))
...
class IXI_train(data.Dataset):
    def __init__(self, opt, train):
        ...
        if self.train:
            ...
            self.crop_size = opt['test_size']
        self.task = opt['task']
        self.scale = int(opt['scale'])
        self.hr_in = opt['hr_in']
        self.align = opt['align']
    ...
    def __getitem__(self, idx):
        ...
        if self.align == True:
            ...
            # ref_GT_img_path = self.GT_paths[idx].replace('T2', 'PD_non_rigid')
        if self.task == 'rec':
            if self.scale == 4:
                mask_path = '/data3/lpc/dataset/IXI/MC_MRI/mask_x4.png'
            elif self.scale == 8:
                mask_path = '/data3/lpc/dataset/IXI/MC_MRI/mask_x8.png'
            else:
                print('Wrong scale for reconstruction!')
        elif self.task == 'sr':
            if self.scale == 2:
                mask_path = '/data3/lpc/dataset/IXI/MCSR/mask_sr_x2.png'
            elif self.scale == 4:
                mask_path = '/data3/lpc/dataset/IXI/MCSR/mask_sr_x4.png'
            elif self.scale == 8:
                mask_path = '/data3/lpc/dataset/IXI/MCSR/mask_sr_x8.png'
            else:
